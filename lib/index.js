...
*
* @param {Number[]|Int8Array|Uint8Array|Uint8ClampedArray|Int16Array|Uint16Array|Int32Array|Uint32Array|Float32Array|Float64Array} y - output vector
...
function lasso( y, x, lambda ) {
	...
	if ( !isArrayLike( y ) ) {
		throw new TypeError( 'lasso()::invalid input argument. The first argument must be array-like. Value: `' + y + '`' );
	}

	if ( isArrayArray( x ) ) {
		x = toMatrix( x );
	} else if ( !isMatrixLike( x ) ) {
		msg = 'lasso()::invalid input argument. The second argument must be a matrix or an array-of-arrays. Value: `' + x + '`';
